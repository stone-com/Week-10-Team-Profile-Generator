//function to create manager card
const createManager = (manager) => {
  return `
    <div class="col-4 mt-4">
        <div class="card h-100">
            <div class="card-header employee">
                <h3>${manager.name}</h3>
                <h4>Manager</h4><i class="material-icons">content_paste</i>
            </div>
            <ul class="card-body list-group-flush">
                <li class="id list-group-item">ID: ${manager.id}</li>
                <li class="email list-group-item">Email: <a href="mailto:${manager.email}">${manager.email}</a></li>
                <li class="office list-group-item">Office Number: ${manager.officeNumber}</li>
            </ul>
        </div>
    </div>
    `;
};

//function to create engineer card
const createEngineer = (engineer) => {
  return `
    <div class="col-4 mt-4">
        <div class="card h-100">
            <div class="card-header employee">
                <h3>${engineer.name}</h3>
                <h4>Engineer</h4><i class="material-icons">laptop_mac</i>
            </div>
            <ul class="card-body list-group-flush">
                <li class="id list-group-item">ID: ${engineer.id}</li>
                <li class="email list-group-item">Email: <a href="mailto:${engineer.email}">${engineer.email}</a></li>
                <li class="github list-group-item">Github: <a href="https://github.com/${engineer.github}">${engineer.github}</a></li>
            </ul>
        </div>
    </div>
    `;
};

//function to create intern card
const createIntern = (intern) => {
  return `
    <div class="col-4 mt-4">
        <div class="card h-100">
            <div class="card-header employee">
                <h3>${intern.name}</h3>
                <h4>Intern</h4><i class="material-icons">assignment_ind</i>
            </div>
            <ul class="card-body list-group-flush">
                <li class="id list-group-item">ID: ${intern.id}</li>
                <li class="email list-group-item">Email:<a href="mailto:${intern.email}">${intern.email}</a></li>
                <li class="school list-group-item">School: ${intern.school}</li>
            </ul>
        </div>
    </div>
    `;
};
//function to create cards for each user submitted employee, then push to an array
// push array to page
const createHTML = (data) => {
  // initialize empty array, will be filled with all employees from user submitted data
  let employeeArray = [];
  //loop through all employees in data from inquirer, create a roll variable for each employee
  for (let i = 0; i < data.length; i++) {
    const employee = data[i];
    const role = employee.getRole();

    // if role is manager, call createManager function
    if (role === 'Manager') {
      const managerCard = createManager(employee);
      //push new card to employeeArray
      employeeArray.push(managerCard);
    }

    // if role is engineer, call createEngineer function
    if (role === 'Engineer') {
      const engineerCard = createEngineer(employee);
      //push new card to employeeArray
      employeeArray.push(engineerCard);
    }

    // if role is intern, call createIntern function
    if (role === 'Intern') {
      const internCard = createIntern(employee);
      //push new card to employeeArray
      employeeArray.push(internCard);
    }
  }

  //turn array into a string with .join method. (needs to be a string since its all HTML)
  const cards = employeeArray.join('');

  //call createTeamPage function passing in string of generated cards, save to variable and return it.
  const generateTeam = createTeamPage(cards);
  return generateTeam;
};

const createTeamPage = (cards) => {
  //return HTML page with links to stylesheet, bootstrap, font awesome, and jquery/bootstrap script tags.
  //all generated cards will be included on the page
  return `
    <!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Team Profile Page</title>
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.11.2/css/all.min.css">
        <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.0/css/bootstrap.min.css" integrity="sha384-9aIt2nRpC12Uk9gS9baDl411NQApFmC26EwAOH8WgZl5MYYxFfc+NcPb1dKGj7Sk" crossorigin="anonymous">
        <link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet">
        <link rel="stylesheet" href="style.css">
    </head>
    <body>
        <header>
            <nav class="navbar" id="navbar">
                <span class="navbar-brand mb-0 h1 w-100 text-center" id="navbar-text">Team Profile</span>
            </nav>
        </header>
        <main>
            <div class="container">
                <div class="row justify-content-center" id="team-cards">
                    <!--Team Cards-->
                   ${cards}
                </div>
            </div>
        </main>
        
    </body>
    <script src="https://code.jquery.com/jquery-3.5.1.slim.min.js" integrity="sha384-DfXdz2htPH0lsSSs5nCTpuj/zy4C+OGpamoFVy38MVBnE+IbbVYUew+OrCXaRkfj" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js" integrity="sha384-Q6E9RHvbIyZFJoft+2mJbHaEWldlvI9IOYy5n3zV9zzTtmI3UksdQRVvoxMfooAo" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.5.0/js/bootstrap.min.js" integrity="sha384-OgVRvuATP1z7JjHLkuOU7Xw704+h835Lr+6QL9UvYjZE3Ipu6Tp75j7Bh/kR0JKI" crossorigin="anonymous"></script>
    </html>
  `;
};

// export createHTML function
module.exports = createHTML;
